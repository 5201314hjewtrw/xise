<template>
  <div class="api-docs" :class="{ 'api-docs--standalone': standalone }">
...
// 保存滚动位置的key
const SCROLL_POSITION_KEY = props.standalone ? 'public_api_docs_scroll_position' : 'admin_api_docs_scroll_position'

onMounted(() => {
  // 根据模式选择滚动容器：standalone模式使用.public-content-body或window，admin模式使用.content-body
  const contentBody = props.standalone 
    ? document.querySelector('.public-content-body') || window
    : document.querySelector('.content-body')
  
  if (contentBody) {
    contentBodyElement.value = contentBody
    const isWindow = contentBody === window

    // 初始化scrollY值
    scrollY.value = isWindow ? window.scrollY : contentBody.scrollTop
...
onBeforeUnmount(() => {
  if (contentBodyElement.value) {
    const isWindow = contentBodyElement.value === window
    const currentScrollPosition = isWindow ? window.scrollY : contentBodyElement.value.scrollTop
...
/* 独立模式下的搜索框位置调整 */
.api-docs--standalone .sticky-search {
  top: 70px;
}
